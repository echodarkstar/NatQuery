# coding: utf-8
from sqlalchemy import ARRAY, Boolean, CHAR, Column, Date, DateTime, Enum, ForeignKey, Index, Integer, LargeBinary, Numeric, SmallInteger, Table, Text, text
...
class Store(Base):
    ...
    manager_staff_id = Column(SmallInteger, nullable=False, unique=True)
...
class Customer(Base):
    ...
    customer_id = Column(Integer, primary_key=True, server_default=text("nextval('customer_customer_id_seq1'::regclass)"))
...
class Rental(Base):
    ...
    staff_id = Column(ForeignKey('staff.staff_id', ondelete='RESTRICT', onupdate='CASCADE'), nullable=False)
    last_update = Column(DateTime(True), nullable=False, server_default=text("now()"))
