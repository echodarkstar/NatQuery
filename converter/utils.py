...
def db_info(db):
    ...
    for table in db_tables:
        mapping[table] = [x[0] for x in db.engine.execute("select column_name from information_schema.columns where table_name = '" + table + "'") if (x[0] != 'last_update' and x[0] != 'special_features' and x[0] != 'address2' and x[0] != 'rental_rate' and x[0] != 'replacement_cost' and x[0] != 'original_language_id' and x[0] != 'create_date' and x[0] != 'activebool'  )]
    return (mapping)
